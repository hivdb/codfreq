FROM python:3.8-alpine as samtools_builder
...
FROM python:3.8-alpine as minimap2_installer
RUN apk add gcc make g++ zlib-dev bzip2-dev xz-dev linux-headers ncurses-dev curl-dev coreutils
...
FROM python:3.8-alpine as pysam_builder
RUN apk add gcc make g++ zlib-dev bzip2-dev xz-dev linux-headers ncurses-dev curl-dev coreutils
RUN echo 'manylinux2014_compatible = True' > /usr/local/lib/python3.8/site-packages/_manylinux.py
RUN pip install 'cython>=0.29.23'
RUN pip install 'pysam==0.15.4'

FROM python:3.8-alpine as py_builder
RUN apk add gcc make g++ zlib-dev bzip2-dev xz-dev linux-headers ncurses-dev curl-dev coreutils
...
COPY --from=py_builder /python-packages/ /usr/local/lib/python3.8/site-packages/
